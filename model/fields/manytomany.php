...
class ManyToManyField extends ChildrenField
{
	...
	public function getConnectorQueries(spitfire\storage\database\Query$parent)
	{
		...
		if ($this->target !== $this->getModel()) {
			...
			foreach ($fields as $field) {
				if ($field->getTarget() === $this->getModel()) {
					...
					foreach ($physical as $p) {
						$route->where($of->queryFieldInstance($route->getQueryTable(), $p), $of->queryFieldInstance($parent->getQueryTable(), $p->getReferencedField()));
					}
				} else {
					$physical = $field->getPhysical();
					foreach ($physical as $p) {
						$query->where($of->queryFieldInstance($route->getQueryTable(), $p), $of->queryFieldInstance($query->getQueryTable(), $p->getReferencedField()));
					}
				}
			}
			...
			foreach ($f1p as $p) {
				$route1->where($of->queryFieldInstance($route1->getQueryTable(), $p), $of->queryFieldInstance($parent->getQueryTable(), $p->getReferencedField()));
			}
			foreach ($f2p as $p) {
				$route2->where($of->queryFieldInstance($route2->getQueryTable(), $p), $of->queryFieldInstance($parent->getQueryTable(), $p->getReferencedField()));
			}
			
			#Exclude repeated results from Route2
			$group = $route2->group(\spitfire\storage\database\RestrictionGroup::TYPE_OR);
			foreach ($f1p as $k => $v) {
				$group->where($of->queryFieldInstance($route2->getQueryTable(), $v), '<>', $of->queryFieldInstance($route2->getQueryTable(), $f2p[$k]));
			}
			...
			foreach ($f2p as $p) {
				$r1group->addRestriction($of->queryFieldInstance($route1->getQueryTable(), $p), $of->queryFieldInstance($query->getQueryTable(), $p->getReferencedField()));
			}
			foreach ($f1p as $p) {
				$r2group->addRestriction($of->queryFieldInstance($route2->getQueryTable(), $p), $of->queryFieldInstance($query->getQueryTable(), $p->getReferencedField()));
			}
